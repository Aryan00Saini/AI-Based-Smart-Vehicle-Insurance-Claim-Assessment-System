...
Exports trained checkpoint to data/models/yolov8s_damage.onnx.
"""
import os
import shutil
...
def train_damage_model(epochs: int = 3, imgsz: int = 640, batch: int = 4):
    ...
    print("[Train] Damage segmentation training complete.")
    best_weights = TRAINING_DIR / "runs" / "damage_segmentation" / "weights" / "best.pt"
    if not best_weights.exists():
        best_weights = TRAINING_DIR / "runs" / "damage_segmentation" / "weights" / "last.pt"
    ...
    if Path(onnx_file).exists():
        ...
        print(f"[Success] Real YOLOv8s-seg damage model deployed to {dest_path}")
